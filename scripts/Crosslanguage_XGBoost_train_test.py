"""
Cross-Language XGBoost Training and Testing
============================================

Script này thực hiện:
1. Đọc dữ liệu đã được gán nhãn (Labelled_Dataset.csv)
2. Đánh giá hiệu suất XGBoost với cross-validation (10 folds)
3. Tìm hyperparameters tối ưu bằng Bayesian Optimization
4. Huấn luyện mô hình cuối cùng với toàn bộ dữ liệu
5. Lưu mô hình đã train vào file .pkl

Yêu cầu:
    - File Labelled_Dataset.csv phải có trong cùng thư mục
    - Đã cài đặt các thư viện: pandas, joblib, xgboost, scikit-learn, psutil
"""
...
from utilities_functions import evaluation_NPM_Pypi_xgb
...
print("="*70)
print("CROSS-LANGUAGE XGBOOST - TRAINING & TESTING")
print("="*70)
print("\n[*] Đang đọc dữ liệu từ Labelled_Dataset.csv...")

try:
    data = pd.read_csv('./Labelled_Dataset.csv', sep=',')
    print(f"[✓] Đọc dữ liệu thành công: {len(data)} samples")
    print(f"    - Số features: {len(data.columns) - 3}")  # trừ 3 cột: Malicious, Package Repository, Package Name
    print(f"    - Malicious: {data['Malicious'].sum()} | Benign: {len(data) - data['Malicious'].sum()}")
    if 'Package Repository' in data.columns:
        print(f"    - Package Repository distribution:")
        print(data['Package Repository'].value_counts().to_string())
...
# ============================================================================
# 2. ĐÁNH GIÁ VÀ TỐI ƯU HÓA XGBOOST
# ============================================================================
...
print("[*] XGBoost tối ưu hóa gradient boosting cho hiệu suất cao...")
print()
...
print("\n" + "-"*70)
print("METRICS TỔNG QUÁT (Overall)")
print("-"*70)
...
print("\n" + "-"*70)
print("METRICS CHO NPM PACKAGES")
print("-"*70)
print(f"Test Precision NPM   : {round(performance.iloc[-8,0],2)} ± {round(performance.iloc[-8,1],2)}%")
print(f"Test Recall NPM      : {round(performance.iloc[-7,0],2)} ± {round(performance.iloc[-7,1],2)}%")
print(f"Test F1-Score NPM    : {round(performance.iloc[-6,0],2)} ± {round(performance.iloc[-6,1],2)}%")
print(f"Test Accuracy NPM    : {round(performance.iloc[-5,0],2)} ± {round(performance.iloc[-5,1],2)}%")

print("\n" + "-"*70)
print("METRICS CHO PYPI PACKAGES")
print("-"*70)
print(f"Test Precision PyPI  : {round(performance.iloc[-4,0],2)} ± {round(performance.iloc[-4,1],2)}%")
print(f"Test Recall PyPI     : {round(performance.iloc[-3,0],2)} ± {round(performance.iloc[-3,1],2)}%")
print(f"Test F1-Score PyPI   : {round(performance.iloc[-2,0],2)} ± {round(performance.iloc[-2,1],2)}%")
print(f"Test Accuracy PyPI   : {round(performance.iloc[-1,0],2)} ± {round(performance.iloc[-1,1],2)}%")
...
print("="*70)
print("[*] Đang train mô hình XGBoost với toàn bộ dữ liệu...")
...
print(f"    - Number of boosting rounds: {hyperparams['n_estimators']}")
print(f"    - Learning rate (eta): {hyperparams['eta']}")
...
joblib_file = os.path.join(models_dir, f'CrossLanguage_XGBoost_{dt}.pkl')
...
print(f"    - Number of boosting rounds: {hyperparams['n_estimators']}")
print(f"    - Max tree depth: {hyperparams['max_depth']}")
print(f"    - Hyperparameters: {hyperparams}")
print(f"    - Training accuracy: {classifier_XGBoost.score(X, Y)*100:.2f}%")
...
if len(feature_names) == len(classifier_XGBoost.feature_importances_):
    feature_importance = sorted(
        zip(feature_names, classifier_XGBoost.feature_importances_),
        key=lambda x: x[1],
        reverse=True
    )
