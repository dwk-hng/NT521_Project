"""
Script Huấn Luyện và Kiểm Tra XGBoost Đơn Ngôn Ngữ JS
Huấn luyện mô hình phân loại XGBoost chỉ trên các gói NPM (JavaScript)
...
def main():
    """
    Hàm chính để huấn luyện và đánh giá XGBoost trên các gói NPM
    """
    print("=" * 80)
    print("HUẤN LUYỆN VÀ KIỂM TRA XGBOOST ĐƠN NGÔN NGỮ JS")
    print("=" * 80)
    print()
    ...
    # Lọc chỉ các gói NPM
    print("\n✓ Đang lọc chỉ các gói NPM...")
    data = data[data['Package Repository'] == 'NPM']
    print(f"  Số gói NPM: {len(data)}")
    print(f"  Độc hại: {len(data[data['Malicious'] == 1])}")
    print(f"  Lành tính: {len(data[data['Malicious'] == 0])}")
    
    if len(data) == 0:
        print("✗ Lỗi: Không tìm thấy gói NPM nào trong tập dữ liệu!")
        return
    
    # Huấn luyện và đánh giá hiệu suất
    print("\n" + "=" * 80)
    print("HUẤN LUYỆN XGBOOST VỚI BAYESIAN OPTIMIZATION")
    print("=" * 80)
    print()
    ...
    print(f"✓ Đang huấn luyện XGBoost trên toàn bộ tập dữ liệu ({len(X)} mẫu)...")
    ...
    joblib_file = os.path.join(models_dir, f'JS_Monolanguage_XGBoost_{dt}.pkl')
